...
template <class data_T, typename CONFIG_T, typename ACT_CONFIG>
...
forward_label0:
    ...
    {
#pragma HLS PIPELINE
        data_T Wxh_dot_x_sum = 0;
        data_T Whh_dot_h_sum = 0;
        for (int j = 0; j < CONFIG_T::n_in; j++)
        {
            data_T Wxh_dot_x = Wxh[i][j] * data[j];
            Wxh_dot_x_sum += Wxh_dot_x;
        }

        for (int j = 0; j < CONFIG_T::n_state; j++)
        {
            data_T Whh_dot_h = Whh[i][j] * h_last[j];
            Whh_dot_h_sum += Whh_dot_h;
        }
        h_tmp_in[i] = Wxh_dot_x_sum + Whh_dot_h_sum + bh[i];
    }
    tanh<data_T, data_T, ACT_CONFIG>(h_tmp_in, h_curr);
    for (int i = 0; i < CONFIG_T::n_state; i++)
    {
#pragma HLS unroll
        h_last[i] = h_curr[i];
